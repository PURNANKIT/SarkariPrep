...
const Practice = () => {
  ...
  const getOptions = (q) => {
    if (!q) return [];
    return lang === "hi"
      ? q.options_hi || q.options_en || q.options || []
      : q.options_en || q.options || q.options_hi || [];
  };
  ...
  const handleSubmit = () => {
    ...
    questions.forEach((q) => {
      ...
      if (typeof q.answer === "number") {
        ...
        if (
          opts[sel] &&
          opts[sel].toLowerCase() === String(q.answer).toLowerCase()
        )
          s++;
      }
    });
      ...
      >
        ...
        >
          ...
          <p style={{ marginBottom: "20px", opacity: 0.9 }}>
            Please login to access the Practice Section.
          </p>
  ...
  // ----------------------------------
  // LOGGED IN USER → SHOW PRACTICE APP
  // ----------------------------------
    ...
    <div className="practice-wrap">
      ...
      <div className="time-question">
        ...
        <span className="question-number">
          Question {curIndex + 1} / {questions.length}
        </span>
      </div>
        ...
        <div>
          ...
          <div className="quiz-actions">
            <div className="left-buttons">
              <button onClick={prev} disabled={curIndex === 0}>
                Prev
              </button>
              <button
                onClick={next}
                disabled={curIndex + 1 >= questions.length}
              >
                Next
              </button>
            </div>
                ...
                return (
                  <li key={i} className={isCorrect ? "correct" : "wrong"}>
                    ...
                    <div className="review-answers">
                      Your answer:{" "}
                      {sel !== undefined ? getOptions(q)[sel] : "Not answered"}
                    </div>
                    <div className="review-answers">
                      Correct answer: {getOptions(q)[correctAnswer]}
                    </div>
                  </li>
                );
              })}
