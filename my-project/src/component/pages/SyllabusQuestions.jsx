...
import { useLocation, useNavigate, useParams } from "react-router-dom";
import "./SyllabusQuestions.css";
import { useUser } from "../context/UserContext"; // ✅ SAME AS QuizPage

const SyllabusQuestions = () => {
  /* ✅ LANGUAGE FROM CONTEXT (NO APP.JSX CHANGE) */
  ...
  useEffect(() => {
    ...
    const fetchById = async () => {
      if (!id) {
        setError(lang === "hi" ? "प्रश्न आईडी नहीं मिली" : "Question id missing");
        return;
      }
      try {
        setLoading(true);
        const res = await fetch(
          `http://localhost:5000/api/question/${encodeURIComponent(id)}`
        );
        if (!res.ok) throw new Error("Not found");
        const data = await res.json();
        ...
        console.error(err);
        setError(lang === "hi" ? "प्रश्न नहीं मिला" : "Question not found");
        setLoading(false);
      }
    };
  ...
  const handleFinish = () => {
    let correct = 0;
    (allQuestions.length ? allQuestions : [question]).forEach((q) => {
  ...
  if (loading)
    return (
      <p className="sq-loading">
        {lang === "hi" ? "प्रश्न लोड हो रहा है..." : "Loading question..."}
      </p>
    );

  if (error)
    return <p className="sq-error">{error}</p>;

  if (!question)
    return (
      <p className="sq-error">
        {lang === "hi" ? "कोई प्रश्न नहीं मिला।" : "No question to display."}
      </p>
    );
  ...
  if (finished) {
    ...
    return (
      <div className="sq-container">
        <div className="sq-card">
          <h2>{lang === "hi" ? "परिणाम" : "Result"}</h2>
          <p>
            {lang === "hi" ? "स्कोर" : "Score"}: {score}/{total}
          </p>

          <button className="sq-btn" onClick={handleBack}>
  ...
  /* ================= NORMAL VIEW ================= */
  const options =
    lang === "hi" && question.options_hi
      ? question.options_hi
      : question.options_en;

  const qText =
    lang === "hi" && question.q_hi ? question.q_hi : question.q_en;
  ...
  return (
    <div className="sq-container">
      <div className="sq-card">
        ...
        <p className="sq-question">{qText}</p>
        ...
        <div className="sq-bottom">
          <button className="sq-btn" onClick={goPrev} disabled={index <= 0}>
            ← {lang === "hi" ? "पिछला" : "Prev"}
          </button>

          {index < allQuestions.length - 1 ? (
            <button className="sq-btn" onClick={goNext}>
              {lang === "hi" ? "अगला" : "Next"} →
            </button>
          ) : (
            <button className="sq-btn finish" onClick={handleFinish}>
