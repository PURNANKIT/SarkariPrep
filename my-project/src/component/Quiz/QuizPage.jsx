...
export default function QuizPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (
        infoKey &&
        jobInfo[infoKey] &&
        Array.isArray(jobInfo[infoKey].questions)
      ) {
      ...
      const stamp = Number(
        localStorage.getItem(ATTEMPT_KEY(normalizedRouteKey)) || 0
      );
      const now = Date.now();
      ...
      if (stamp && now < stamp + RETAKE_MS) {
        ...
        if (saved) {
          try {
            ...
            setQuestions(
              attempt.questions || sampleQuestions(pool, TOTAL_QUESTIONS)
            );
      ...
      >
        <div
          style={{
            ...
            boxShadow:
              "0 12px 35px rgba(0,0,0,0.35), 0 0 25px rgba(0,255,255,0.3)",
            color: "#fff",
            textAlign: "center",
        ...
        >
          ...
          <p style={{ marginBottom: "20px", opacity: 0.95, fontSize: "18px" }}>
            You're not logged in. Please sign up / login.
          </p>
          ...
          <button
            ...
            onMouseOut={(e) =>
              (e.target.style.background = "rgba(0,255,255,0.9)")
            }
          >
            Go to Signup
          </button>
    ...
    return (
      <main className="quiz-wrap">
        <div className="quiz-empty">
          <p>
            Unknown exam: <strong>{rawRoute || "N/A"}</strong>
          </p>
  ...
  if (loading)
    return (
      <main className="quiz-wrap">
        <div className="quiz-empty">Loading quiz...</div>
      </main>
    );
  if (!questions || !questions.length) {
    return (
      <main className="quiz-wrap">
        <div className="quiz-empty">
          <h2>
            {(jobInfo[infoKey] && jobInfo[infoKey].title) || infoKey} — Quiz
          </h2>
  ...
  const getOptions = (qq) => {
    if (!qq) return [];
    if (lang === "hi")
      return qq.options_hi || qq.options_en || qq.options || [];
  ...
  return (
    <main className="quiz-wrap">
      <header className="quiz-header">
        <div>
          <h2>
            {(jobInfo[infoKey] && jobInfo[infoKey].title) || infoKey} —{" "}
            {lang === "hi" ? "क्विज़" : "Quiz"}
          </h2>
          <div className="quiz-desc">
            {(jobInfo[infoKey] && jobInfo[infoKey].desc) || ""}
          </div>
        </div>
        ...
        <div className="quiz-timer">
          {finished ? (
            <div className="cooldown-note">
              {" "}
              {lang === "hi"
                ? "पुनः प्रयास के लिए शेष समय"
                : "Retry available in"}{" "}
              <strong>{formatTime(cooldownRemaining)}</strong>
            </div>
          ) : (
            <div className="q-progress">
              {lang === "hi" ? "प्रश्न" : "Question"} {cur + 1} /{" "}
              {questions.length}
            </div>
          )}
        </div>
          ...
          <div className="quiz-actions">
            <button
              className={`btn primary ${
                answers[cur] === undefined ? "disabled" : ""
              }`}
              onClick={handleNext}
                ...
                return (
                  ...
                  >
                    <div className="review-q">
                      <strong>{idx + 1}.</strong>{" "}
                      {lang === "hi"
                        ? qq.q_hi || qq.q_en || qq.q
                        : qq.q_en || qq.q || qq.q_hi}
                    </div>
                    <div className="review-a">
                      <div>
                        {lang === "hi" ? "आपका उत्तर:" : "Your answer:"}{" "}
                        <strong>
                          {sel === undefined || sel === null
                            ? lang === "hi"
                              ...
                              : "Not answered"
                            : lang === "hi"
                            ? (qq.options_hi && qq.options_hi[sel]) ||
                              qq.options_en[sel] ||
                              qq.options[sel]
                            : (qq.options_en && qq.options_en[sel]) ||
                              qq.options[sel] ||
                              qq.options_hi[sel]}
                        </strong>
                      </div>
                      {!isCorrect && (
                        <div className="corr">
                          ...
                          <strong>
                            {lang === "hi"
                              ? (qq.options_hi && qq.options_hi[correct]) ||
                                qq.options_en[correct]
                              : (qq.options_en && qq.options_en[correct]) ||
                                qq.options_hi[correct]}
                          </strong>
                        </div>
                      )}
                    </div>
          ...
          <div className="quiz-actions">
            ...
            {cooldownRemaining > 0 && (
              <div className="cooldown-msg">
                {lang === "hi" ? "आप " : "You can retry after: "}{" "}
                <strong>{formatTime(cooldownRemaining)}</strong>
              </div>
            )}
          </div>
